...
- **Email Handling**: Robust email notification system with status tracking and resend capabilities for join requests, password reset emails with secure token-based authentication, and welcome emails sent to all new users upon signup (across all authentication methods).
