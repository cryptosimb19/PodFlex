...
- **Core Features**:
    ...
    - **Internal Messaging**: Pod leaders can send direct messages to individual members or broadcast group messages to all pod members. Members can view and reply to conversations. Unread message count badge shown in navigation. Real-time delivery via WebSocket (`/ws/messages`) using `ws` package in `noServer` mode — the server broadcasts full message objects to connected clients so messages appear instantly for both sender and receiver without any HTTP refetch. Sender input is cleared immediately on send with optimistic UI update. WebSocket server: `server/messagingWS.ts`.
    - **PodAgent**: Conversational AI assistant available to all authenticated users (leaders and seekers) as a persistent floating chat widget (bottom-right corner). Pod seekers can browse pods, check join request status, and get pod details. Pod leaders can view pending join requests, approve/decline requests, and check pod members — all through natural language conversation. Uses OpenAI function calling via Replit AI Integrations. Component: `client/src/components/PodAgent.tsx`. API: `POST /api/agent/chat`. Backend logic: `server/podAgent.ts`.
